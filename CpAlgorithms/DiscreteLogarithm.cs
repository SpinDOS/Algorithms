...
namespace CpAlgorithms
{
    public static class DiscreteLogarithm
    {
        public static uint? SimpleSolve(uint a, uint b, uint m)
        {
            ...
            for (var p = 1U; p <= n; p++)
                leftFunction.TryAdd((uint) BinaryExponentiation.FastArithmetics.ModuloPow(a, n * p, m), p);

            for (var q = 0U; q <= n; q++)
            {
                var val = (b * BinaryExponentiation.FastArithmetics.ModuloPow(a, q, m)) % m;
        ...
        public static uint? OptimizedSolve(uint a, uint b, uint m)
        {
            ...
            var an = BinaryExponentiation.FastArithmetics.ModuloPow(a, n, m);
            var leftVal = an;
            for (var p = 1U; p <= n; p++)
            {
